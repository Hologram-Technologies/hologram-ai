//! Neg operation translator.

use crate::proto::NodeProto;
use crate::translators::{InputRequirement, OnnxTranslator, TranslationError};
use hologram::ir::{GraphBuilder, NodeIndex, NodeOp};

/// Translator for ONNX Neg operation.
///
/// Neg(x) = -x (element-wise negation)
#[derive(Debug, Default)]
pub struct NegTranslator;

impl OnnxTranslator for NegTranslator {
    ...
    fn constant_fold(&self, _node: &NodeProto, constant_inputs: &[&[u8]]) -> Option<Vec<u8>> {
        ...
        let result: Vec<f32> = floats.iter().map(|x| -x).collect();
...
mod tests {
    ...
    #[test]
    fn test_neg_single_input() {
        let translator = NegTranslator;
    ...
    #[test]
    fn test_neg_constant_fold_basic() {
        let translator = NegTranslator;
        ...
        assert_eq!(output, &[2.0, 1.0, 0.0, -1.0, -2.0]);
    }

    #[test]
    fn test_neg_constant_fold_all_negative() {
        let translator = NegTranslator;
        ...
        let output_bytes = result.unwrap();
        let output: &[f32] = bytemuck::cast_slice(&output_bytes);
        assert_eq!(output, &[5.0, 3.0, 1.0, 0.1]);
    }

    #[test]
    fn test_neg_constant_fold_all_positive() {
        let translator = NegTranslator;
    ...
    #[test]
    fn test_neg_constant_fold_empty() {
        let translator = NegTranslator;
    ...
    #[test]
    fn test_neg_no_inputs() {
        let translator = NegTranslator;
        let err = translator.input_requirement().validate(0, "Neg");
        assert!(err.is_err());
        assert!(matches!(
            err.unwrap_err(),
    ...
    #[test]
    fn test_neg_too_many_inputs() {
        let translator = NegTranslator;
        let err = translator.input_requirement().validate(2, "Neg");
        assert!(err.is_err());
    }
}
