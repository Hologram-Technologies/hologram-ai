//! Abs operation translator.

use crate::proto::NodeProto;
use crate::translators::{InputRequirement, OnnxTranslator, TranslationError};
use hologram::ir::{GraphBuilder, NodeIndex, NodeOp};

/// Translator for ONNX Abs operation.
///
/// Abs(x) = |x| (element-wise absolute value)
#[derive(Debug, Default)]
pub struct AbsTranslator;

impl OnnxTranslator for AbsTranslator {
    ...
    fn constant_fold(&self, _node: &NodeProto, constant_inputs: &[&[u8]]) -> Option<Vec<u8>> {
        ...
        let result: Vec<f32> = floats.iter().map(|x| x.abs()).collect();
...
mod tests {
    ...
    #[test]
    fn test_abs_constant_fold_basic() {
        let translator = AbsTranslator;
    ...
    #[test]
    fn test_abs_constant_fold_all_negative() {
        let translator = AbsTranslator;
    ...
    #[test]
    fn test_abs_constant_fold_all_positive() {
        let translator = AbsTranslator;
    ...
    #[test]
    fn test_abs_constant_fold_empty() {
        let translator = AbsTranslator;
    ...
    #[test]
    fn test_abs_no_inputs() {
        let translator = AbsTranslator;
        let err = translator.input_requirement().validate(0, "Abs");
        assert!(err.is_err());
        assert!(matches!(
            err.unwrap_err(),
    ...
    #[test]
    fn test_abs_too_many_inputs() {
        let translator = AbsTranslator;
        let err = translator.input_requirement().validate(2, "Abs");
        assert!(err.is_err());
    }
}
