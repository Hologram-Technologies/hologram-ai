...
mod tests {
    ...
    #[test]
    fn test_add_no_inputs() {
        let translator = AddTranslator;
        let err = translator.input_requirement().validate(0, "Add");
        assert!(err.is_err());
    }

    #[test]
    fn test_add_one_input() {
        let translator = AddTranslator;
        let err = translator.input_requirement().validate(1, "Add");
        assert!(err.is_err());
    }

    #[test]
    fn test_add_too_many_inputs() {
        let translator = AddTranslator;
        let err = translator.input_requirement().validate(3, "Add");
        assert!(err.is_err());
    }
}
