...
///
/// ReduceMax computes the maximum of elements along the specified axes.
///
/// # Attributes
///
...
/// # Outputs
///
/// - `reduced`: Reduced tensor containing maximum values
#[derive(Debug, Default)]
pub struct ReduceMaxTranslator;

impl OnnxTranslator for ReduceMaxTranslator {
    ...
    fn translate(
        ...
        let result = builder
            .unary(NodeOp::ReduceMax { axes, keepdims }, data)
...
mod tests {
    ...
    fn make_node() -> NodeProto {
        NodeProto {
            name: "reduce_max_test".to_string(),
    ...
    #[test]
    fn test_reduce_max_single_axis() {
        let translator = ReduceMaxTranslator;
    ...
    #[test]
    fn test_reduce_max_negative_axis() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[3, 4, 5]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_first_axis() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[8, 16, 32]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_multiple_axes() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4, 5]), DType::F32);

        let result = translator.translate(&make_node_with_axes(vec![1, 2]), &[x], &mut builder);
        assert!(result.is_ok());
        assert_eq!(result.unwrap().len(), 1);
    }

    #[test]
    fn test_reduce_max_all_axes_explicit() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_max_keepdims_true() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_max_keepdims_false() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[5, 10, 15]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_default_keepdims() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_max_no_axes_reduce_all() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_noop_with_empty_axes() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
    ...
    fn test_reduce_max_1d() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[256]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_2d_row() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[32, 64]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_4d_spatial() {
        let translator = ReduceMaxTranslator;
        let mut builder = GraphBuilder::new();
        // Global max pooling style
        let x = builder.input("x", Shape::static_shape(&[1, 64, 14, 14]), DType::F32);
    ...
    #[test]
    fn test_reduce_max_no_inputs() {
        let translator = ReduceMaxTranslator;
        let err = translator.input_requirement().validate(0, "ReduceMax");
        assert!(err.is_err());
        assert!(matches!(
            err.unwrap_err(),
    ...
    #[test]
    fn test_reduce_max_too_many_inputs() {
        let translator = ReduceMaxTranslator;
        let err = translator.input_requirement().validate(3, "ReduceMax");
        assert!(err.is_err());
    }

    #[test]
    fn test_reduce_max_input_requirement() {
        let translator = ReduceMaxTranslator;
        assert_eq!(
