//! ReduceMin operation translator.
...
/// Translator for ONNX ReduceMin operation.
///
/// ReduceMin computes the minimum of elements along the specified axes.
///
/// # Attributes
///
...
/// # Outputs
///
/// - `reduced`: Reduced tensor containing minimum values
#[derive(Debug, Default)]
pub struct ReduceMinTranslator;

impl OnnxTranslator for ReduceMinTranslator {
    ...
    fn translate(
        ...
        let result = builder
            .unary(NodeOp::ReduceMin { axes, keepdims }, data)
...
mod tests {
    ...
    fn make_node() -> NodeProto {
        NodeProto {
            name: "reduce_min_test".to_string(),
    ...
    #[test]
    fn test_reduce_min_single_axis() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[3, 3, 3]), DType::F32);
    ...
    #[test]
    fn test_reduce_min_negative_axis() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 4, 6]), DType::F32);
    ...
    #[test]
    fn test_reduce_min_middle_axis() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_min_multiple_axes() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_min_all_axes_explicit() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_min_last_two_axes() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4, 5]), DType::F32);
    ...
    #[test]
    fn test_reduce_min_keepdims_true() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[4, 5, 6]), DType::F32);

        let result = translator.translate(&make_node_with_attrs(vec![2], 1), &[x], &mut builder);
        assert!(result.is_ok());
    }

    #[test]
    fn test_reduce_min_keepdims_false() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[4, 5, 6]), DType::F32);

        let result = translator.translate(&make_node_with_attrs(vec![2], 0), &[x], &mut builder);
        assert!(result.is_ok());
    }

    #[test]
    fn test_reduce_min_default_keepdims() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[6, 6]), DType::F32);
    ...
    #[test]
    fn test_reduce_min_no_axes_reduce_all() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_min_noop_with_empty_axes() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[4, 5]), DType::F32);
    ...
    fn test_reduce_min_1d() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[128]), DType::F32);
    ...
    #[test]
    fn test_reduce_min_2d_row() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[16, 32]), DType::F32);
    ...
    #[test]
    fn test_reduce_min_5d() {
        let translator = ReduceMinTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[1, 2, 3, 4, 5]), DType::F32);

        let result = translator.translate(&make_node_with_axes(vec![2, 4]), &[x], &mut builder);
        assert!(result.is_ok());
    }
    ...
    #[test]
    fn test_reduce_min_no_inputs() {
        let translator = ReduceMinTranslator;
        let err = translator.input_requirement().validate(0, "ReduceMin");
        assert!(err.is_err());
        assert!(matches!(
            err.unwrap_err(),
    ...
    #[test]
    fn test_reduce_min_too_many_inputs() {
        let translator = ReduceMinTranslator;
        let err = translator.input_requirement().validate(3, "ReduceMin");
        assert!(err.is_err());
    }

    #[test]
    fn test_reduce_min_input_requirement() {
        let translator = ReduceMinTranslator;
        assert_eq!(
