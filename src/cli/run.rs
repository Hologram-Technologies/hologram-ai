    ...
    match name {
        ...
        "tokenize" | "encode_text" => {
            ...
            if let Some(ref tokenizer) = ctx.tokenizer {
                ...
                let attention_mask: Vec<f32> = token_ids.iter()
                ...
                info!("  Tokenized to {} tokens (vocab size: {})", token_ids.len(), tokenizer.vocab_size());
                info!("  First 10 input tokens: {:?}", &token_ids[..token_ids.len().min(10)]);
            } else {
        ...
        "sample_token" => {
            ...
            let vocab_size = ctx.tokenizer.as_ref()
                .map(|t| t.vocab_size())
        ...
        "generate" => {
            ...
            for step in 0..max_new_tokens {
                ...
                if step == 0 {
                    ...
                    info!("    Last 10 logits: {:?}", &logits[logits.len().saturating_sub(10)..]);

                    // Check for non-zero values
                    let non_zero_count = logits.iter().filter(|&&x| x != 0.0).count();
                    let (min_val, max_val) = logits.iter()
                ...
                if step < 3 {  // Only for first 3 steps to avoid spam
                    let mut top_tokens: Vec<(usize, f32)> = scaled.iter()
                        .enumerate()
                        .map(|(idx, &val)| (idx, val))
                        .collect();
                    ...
                    for (rank, (token_id, logit)) in top_tokens.iter().take(5).enumerate() {
                        ...
                        info!("      {}. Token {} ({}): logit={:.2}", rank + 1, token_id, token_str, logit);
                    }
                }
        ...
        "decode" | "detokenize" => {
            ...
            if let Some(ref tokenizer) = ctx.tokenizer {
                ...
                info!("  First 10 generated tokens: {:?}", &tokens_u32[..tokens_u32.len().min(10)]);
...
fn execute_model_stage(
    ...
    use crate::runtime::{ModelExecutor, Tensor, infer_tensor_shape, infer_tensor_dtype};
        ...
        && let Some(tensor) = output_tensors.get(output_name)
    {
        ...
        let non_zero = data_vec.iter().filter(|&&x| x != 0.0 && !x.is_nan()).count();
        let has_nan = data_vec.iter().any(|x| x.is_nan());
        let (min_val, max_val) = data_vec.iter()
            .filter(|x| !x.is_nan())
            .fold((f32::INFINITY, f32::NEG_INFINITY), |(min, max), &val| {
                (min.min(val), max.max(val))
            });
        debug!("  RAW {}: {} elements, {} non-zero, {} NaN, range [{:.6}, {:.6}]",
            output_name, data_vec.len(), non_zero, if has_nan { "HAS" } else { "no" }, min_val, max_val);
    ...
    debug!("  Available outputs from model: {:?}", output_tensors.keys().collect::<Vec<_>>());
            ...
            OutputHandlerType::Json => {
                ...
                match data {
                    ...
                    OutputData::Audio(sample_rate, samples) => {
                        ...
                        std::fs::write(&output_path, json)?;
                        info!("  Audio output: {} Hz, {} samples", sample_rate, samples.len());
                    }
                }
            }
