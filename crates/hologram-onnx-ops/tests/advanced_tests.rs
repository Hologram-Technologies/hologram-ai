...
use hologram_onnx_ops::{
    translate_matmul, translate_add, translate_gelu, translate_sigmoid,
    translate_layer_normalization,
...
fn test_bert_encoder_block_concrete() {
    ...
    shapes.insert("input".to_string(), concrete_shape(&[batch, seq_len, hidden]));
    ...
    shapes.insert("ff1_weight".to_string(), concrete_shape(&[hidden, intermediate]));
    shapes.insert("ff2_weight".to_string(), concrete_shape(&[intermediate, hidden]));
    shapes.insert("attention_out".to_string(), concrete_shape(&[batch, seq_len, hidden]));
    ...
    shapes.insert("ff1_out".to_string(), concrete_shape(&[batch, seq_len, intermediate]));
    ...
    eprintln!("BERT encoder block (concrete): {} IR nodes", func.body.len());
}
...
fn test_bert_encoder_symbolic_shapes() {
    ...
    assert!(!input_shape.is_fully_concrete(), "Input shape should be symbolic");
    ...
    eprintln!("BERT encoder symbolic shapes: verified batch={:?}, seq_len={:?}",
              dims[0], dims[1]);
}
...
fn test_bert_attention_score_shapes() {
    ...
    assert_eq!(q_shape.dims()[2], Dim::Concrete(seq_len), "Sequence dimension should match");

    eprintln!("BERT attention scores: [{}, {}, {}, {}]",
...
fn test_gpt_decoder_block_concrete() {
    ...
    let mut shapes = HashMap::new();
    shapes.insert("input".to_string(), concrete_shape(&[batch, seq_len, hidden]));
    shapes.insert("qkv_weight".to_string(), concrete_shape(&[hidden, 3 * hidden]));
...
fn test_gpt_autoregressive_shapes() {
    ...
    eprintln!("GPT autoregressive shapes verified: input={:?}, kv_cache=4D symbolic",
              input_shape.dims());
}
...
fn test_lstm_cell_concrete() {
    ...
    shapes.insert("W_x".to_string(), concrete_shape(&[input_size, 4 * hidden_size]));
    shapes.insert("W_h".to_string(), concrete_shape(&[hidden_size, 4 * hidden_size]));
    ...
    // Add projections
    shapes.insert("x_proj".to_string(), concrete_shape(&[batch, 4 * hidden_size]));
    shapes.insert("h_proj".to_string(), concrete_shape(&[batch, 4 * hidden_size]));
...
fn test_lstm_variable_sequence_length() {
    ...
    eprintln!("LSTM variable seq shapes: input={:?}, output={:?}",
...
fn test_lstm_full_operation() {
    ...
    eprintln!("LSTM full operation: {} IR nodes (decomposed)", func.body.len());
}
...
fn test_loop_optimization_attention_design() {
    ...
    assert!(savings > 100.0, "LOOP should provide >100x memory savings for seq=1024");
...
fn test_loop_optimization_lstm_design() {
    ...
    assert_eq!(savings, seq_len as f64, "LOOP savings should equal sequence length");
...
fn test_bert_pipeline_shapes() {
    ...
    assert_eq!(embed_shape, layer_output_shape, "Layer should preserve shape");
    assert_eq!(layer_output_shape, output_shape, "Output shape should match");
...
fn test_attention_decomposition() {
    ...
    assert!(result.is_ok(), "Attention should succeed with decomposition");

    builder.set_output(result.unwrap());
    let func = builder.build();

    // Attention decomposes to: transpose, matmul, scale, softmax, matmul
    assert!(func.body.len() >= 5, "Attention should have at least 5 IR nodes");
...
fn test_attention_with_mask() {
    ...
    assert!(func.body.len() >= 6, "Masked attention should have at least 6 IR nodes");
...
fn test_attention_symbolic_shapes() {
    ...
    let mut shapes = HashMap::new();
    shapes.insert("Q".to_string(), symbolic_batch_seq_shape("batch", "seq", &[hidden]));
    shapes.insert("K".to_string(), symbolic_batch_seq_shape("batch", "seq", &[hidden]));
    shapes.insert("V".to_string(), symbolic_batch_seq_shape("batch", "seq", &[hidden]));
    ...
    assert!(result.is_ok(), "Attention with symbolic shapes should succeed");
...
fn test_multi_head_attention_decomposition() {
    ...
    eprintln!("MultiHeadAttention decomposition: {} IR nodes", func.body.len());
}
...
fn test_bert_encoder_layer_complete() {
    ...
    assert!(func.body.len() >= 15, "BERT encoder layer should have many IR nodes");
...
fn test_bert_symbolic_batch_seq() {
    ...
    assert!(weight_shape.is_fully_concrete(), "Weights should be concrete");
...
fn test_gpt_decoder_layer_complete() {
    ...
    let ln_out = translate_layer_normalization(&[residual, gamma, beta], &ln_attrs, &shapes, &mut builder);
    assert!(ln_out.is_ok());
    let ln_out = ln_out.unwrap();

    // Step 3: Feed-forward network
    shapes.insert("ln_out".to_string(), concrete_shape(&[batch, seq_len, hidden]));
    ...
    assert!(func.body.len() >= 20, "GPT decoder layer should have many IR nodes");
...
fn test_gpt_autoregressive_generation() {
    ...
    eprintln!("GPT autoregressive: query=[batch, 1, {}], cache=[batch, {}, past_len, {}]",
...
fn test_lstm_sequence_processing() {
    ...
    eprintln!("LSTM sequence processing: {} IR nodes (seq_len={})", func.body.len(), seq_len);
}
...
fn test_lstm_bidirectional() {
    ...
    let x = builder.add_input("X", f32_tensor(&[seq_len, batch, input_size]));
    let w = builder.add_input("W", f32_tensor(&[num_directions, 4 * hidden_size, input_size]));
    let r = builder.add_input("R", f32_tensor(&[num_directions, 4 * hidden_size, hidden_size]));
...
fn test_lstm_symbolic_sequence() {
    ...
    eprintln!("LSTM symbolic sequence: input=[seq_len, batch, {}], output=[seq_len, 1, batch, {}]",
