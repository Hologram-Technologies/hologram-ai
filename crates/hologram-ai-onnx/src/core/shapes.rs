...
//!
//! ```
//! use hologram_ai_onnx::core::{Dim, SymbolicShape};
//!
...
impl SymbolicShape {
    ...
    ///
    /// ```
    /// use hologram_ai_onnx::core::SymbolicShape;
    ///
    ...
    /// ```no_run
    /// use hologram_ai_onnx::core::SymbolicShape;
    /// use hologram_ai_onnx::core::parse_model;
    ///
...
mod tests {
    ...
    fn test_concrete_shape() {
        ...
        assert_eq!(shape.dims()[0], Dim::Static(2));
        assert_eq!(shape.dims()[1], Dim::Static(3));
        assert_eq!(shape.dims()[2], Dim::Static(4));
    }
    ...
    fn test_symbolic_shape() {
        ...
        assert!(shape.is_partially_symbolic());
        assert!(matches!(&shape.dims()[0], Dim::Symbolic(name) if name == "batch"));
        assert!(matches!(&shape.dims()[1], Dim::Symbolic(name) if name == "seq_len"));
    }
    ...
    fn test_mixed_shape() {
        ...
        assert!(matches!(&shape.dims()[0], Dim::Symbolic(name) if name == "batch"));
        assert_eq!(shape.dims()[1], Dim::Static(224));
        assert_eq!(shape.dims()[2], Dim::Static(224));
        assert_eq!(shape.dims()[3], Dim::Static(3));
    }

    #[test]
    fn test_new_shape() {
        let shape = SymbolicShape::new(vec![Dim::Symbolic("N".into()), Dim::Static(10)]);
        assert_eq!(shape.rank(), 2);
        assert!(shape.is_partially_symbolic());
    }
    ...
    fn test_transpose() {
        ...
        assert_eq!(result.rank(), 3);
        assert_eq!(result.dims()[0], Dim::Static(4));
        assert_eq!(result.dims()[1], Dim::Static(2));
        assert_eq!(result.dims()[2], Dim::Static(3));

        // Default (reverse)
        let result = shape.infer_transpose(None).unwrap();
        assert_eq!(result.rank(), 3);
        assert_eq!(result.dims()[0], Dim::Static(4));
        assert_eq!(result.dims()[1], Dim::Static(3));
        assert_eq!(result.dims()[2], Dim::Static(2));
    }
    ...
    fn test_reshape() {
        ...
        assert_eq!(result.rank(), 2);
        assert_eq!(result.dims()[0], Dim::Static(6));
        assert_eq!(result.dims()[1], Dim::Static(4));
    }
