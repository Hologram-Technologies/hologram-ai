...
///
/// ReduceSum computes the sum of elements along the specified axes.
///
/// # Attributes
///
/// - `axes`: int array (default: reduce all axes) - Axes along which to reduce.
///   In ONNX opset 13+, axes can also be provided as a second input.
...
/// - `axes` (optional, opset 13+): Axes along which to reduce
///
/// # Outputs
///
/// - `reduced`: Reduced tensor
#[derive(Debug, Default)]
pub struct ReduceSumTranslator;

impl OnnxTranslator for ReduceSumTranslator {
    fn onnx_op_type(&self) -> &'static str {
        "ReduceSum"
    }

    fn input_requirement(&self) -> InputRequirement {
        // data is required, axes is optional as second input (opset 13+)
    ...
    fn translate(
        ...
        let result = builder
            .unary(NodeOp::ReduceSum { axes, keepdims }, data)
...
mod tests {
    ...
    fn make_node() -> NodeProto {
        NodeProto {
            name: "reduce_sum_test".to_string(),
    ...
    fn test_reduce_sum_single_axis() {
        let translator = ReduceSumTranslator;
    ...
    #[test]
    fn test_reduce_sum_negative_axis() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4]), DType::F32);
    ...
    #[test]
    fn test_reduce_sum_first_axis() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[10, 20, 30]), DType::F32);
    ...
    fn test_reduce_sum_last_axis() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[5, 10]), DType::F32);
    ...
    fn test_reduce_sum_multiple_axes() {
        ...
        let result = translator.translate(&make_node_with_axes(vec![1, 3]), &[x], &mut builder);
        assert!(result.is_ok());
        assert_eq!(result.unwrap().len(), 1);
    }

    #[test]
    fn test_reduce_sum_all_axes_explicit() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4]), DType::F32);
    ...
    #[test]
    fn test_reduce_sum_mixed_positive_negative_axes() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4, 5]), DType::F32);
    ...
    #[test]
    fn test_reduce_sum_default_keepdims() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
    ...
    #[test]
    fn test_reduce_sum_no_axes_reduce_all() {
        let translator = ReduceSumTranslator;
        let mut builder = GraphBuilder::new();
        let x = builder.input("x", Shape::static_shape(&[2, 3, 4]), DType::F32);
    ...
    #[test]
    fn test_reduce_sum_no_inputs() {
        let translator = ReduceSumTranslator;
        let err = translator.input_requirement().validate(0, "ReduceSum");
        assert!(err.is_err());
        assert!(matches!(
            err.unwrap_err(),
    ...
    #[test]
    fn test_reduce_sum_too_many_inputs() {
        let translator = ReduceSumTranslator;
        let err = translator.input_requirement().validate(3, "ReduceSum");
        assert!(err.is_err());
    }

    #[test]
    fn test_reduce_sum_input_requirement() {
        let translator = ReduceSumTranslator;
        assert_eq!(
