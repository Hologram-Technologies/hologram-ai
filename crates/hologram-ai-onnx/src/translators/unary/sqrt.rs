//! Sqrt operation translator.

use crate::proto::NodeProto;
use crate::translators::{InputRequirement, OnnxTranslator, TranslationError};
use hologram::ir::{GraphBuilder, NodeIndex, NodeOp};

/// Translator for ONNX Sqrt operation.
///
/// Sqrt(x) = x^0.5 (element-wise square root)
#[derive(Debug, Default)]
pub struct SqrtTranslator;

impl OnnxTranslator for SqrtTranslator {
    ...
    fn constant_fold(&self, _node: &NodeProto, constant_inputs: &[&[u8]]) -> Option<Vec<u8>> {
        ...
        let result: Vec<f32> = floats.iter().map(|x| x.sqrt()).collect();
...
mod tests {
    ...
    #[test]
    fn test_sqrt_single_input() {
        let translator = SqrtTranslator;
    ...
    #[test]
    fn test_sqrt_constant_fold_basic() {
        let translator = SqrtTranslator;
        let input: Vec<f32> = vec![1.0, 4.0, 9.0, 16.0, 25.0];
    ...
    #[test]
    fn test_sqrt_constant_fold_zero() {
        let translator = SqrtTranslator;
    ...
    #[test]
    fn test_sqrt_constant_fold_fractional() {
        let translator = SqrtTranslator;
        let input: Vec<f32> = vec![0.25, 0.5, 2.0];
        let input_bytes: &[u8] = bytemuck::cast_slice(&input);
        ...
        assert!((output[1] - std::f32::consts::FRAC_1_SQRT_2).abs() < 1e-5);
        assert!((output[2] - std::f32::consts::SQRT_2).abs() < 1e-5);
    }

    #[test]
    fn test_sqrt_constant_fold_empty() {
        let translator = SqrtTranslator;
    ...
    #[test]
    fn test_sqrt_no_inputs() {
        let translator = SqrtTranslator;
        let err = translator.input_requirement().validate(0, "Sqrt");
        assert!(err.is_err());
        assert!(matches!(
            err.unwrap_err(),
    ...
    #[test]
    fn test_sqrt_too_many_inputs() {
        let translator = SqrtTranslator;
        let err = translator.input_requirement().validate(2, "Sqrt");
        assert!(err.is_err());
    }
}
