...
impl OnnxTranslator for RangeTranslator {
    ...
    fn translate(
        ...
        let constant_data = {
            let start_node = builder.graph().node(inputs[0]).ok_or_else(|| {
                TranslationError::IrBuilder("Range: start input not found".to_string())
            })?;
            let limit_node = builder.graph().node(inputs[1]).ok_or_else(|| {
                TranslationError::IrBuilder("Range: limit input not found".to_string())
            })?;
            let delta_node = builder.graph().node(inputs[2]).ok_or_else(|| {
                TranslationError::IrBuilder("Range: delta input not found".to_string())
            })?;
...
impl RangeTranslator {
    ...
    fn compute_range(
        ...
        match (start_data, limit_data, delta_data) {
            (
                ConstantData::I64(start_vec),
                ConstantData::I64(limit_vec),
                ConstantData::I64(delta_vec),
            ) => Self::compute_range_i64(start_vec, limit_vec, delta_vec, builder),
            (
                ConstantData::I32(start_vec),
                ConstantData::I32(limit_vec),
                ConstantData::I32(delta_vec),
            ) => Self::compute_range_i32(start_vec, limit_vec, delta_vec, builder),
            (
                ConstantData::F32(start_vec),
                ConstantData::F32(limit_vec),
                ConstantData::F32(delta_vec),
            ) => Self::compute_range_f32(start_vec, limit_vec, delta_vec, builder),
            (
                ConstantData::F64(start_vec),
                ConstantData::F64(limit_vec),
                ConstantData::F64(delta_vec),
            ) => Self::compute_range_f64(start_vec, limit_vec, delta_vec, builder),
            _ => Err(TranslationError::IrBuilder(
                "Range: type mismatch or unsupported data types (all inputs must have same type)"
                    .to_string(),
            )),
        }
    }
    ...
    fn compute_range_i64(
        ...
        if start_vec.is_empty() || limit_vec.is_empty() || delta_vec.is_empty() {
            return Err(TranslationError::IrBuilder(
                "Range: inputs must be non-empty".to_string(),
            ));
        }
