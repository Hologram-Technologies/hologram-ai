...
mod tests {
    ...
    fn test_mul_constant_fold() {
        ...
        assert_eq!(output, &[10.0, 18.0, 28.0]);
    }

    #[test]
    fn test_mul_invalid_inputs() {
        let translator = MulTranslator;
        assert!(translator.input_requirement().validate(0, "Mul").is_err());
        assert!(translator.input_requirement().validate(1, "Mul").is_err());
        assert!(translator.input_requirement().validate(3, "Mul").is_err());
    }
}
