...
pub fn lower(
    ...
    // All weights are registered as f32. Quantization is handled by the
    // post-lowering resolve_encodings() pass (Plan 077 — UOR encoding).
        ...
        match dispatch(&node.op) {
            ...
            DispatchTarget::MultiOutput => {
                ...
                } = &node.op
                {
                    ...
                    // Step 2: Emit N MatMul nodes, one per projection (all f32).
                    // Quantization is handled by the post-lowering resolve_encodings() pass.
