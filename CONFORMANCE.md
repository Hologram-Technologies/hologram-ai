...
| **PV-7** | **Generation memory is bounded by the window, not the token count.** A long autoregressive run holds steady at one window's working set regardless of how many tokens are produced — the content-addressed pool rotates generations at the *walk* boundary (one `execute`), with no byte budget or hardcoded cap, so resident transient is the last two walks (current + previous-for-elision) and scales only with the model/window. Observed (SmolLM2-135M): 50 tokens in a 64-window held flat at ~2.5 GB (was ~5.8 GB and climbing ~90 MB/token before the fix); generation grows the window (64→128→…) to the model's real context and continues, peak ~2.7 GB at 128. The eviction is structure-driven, not a magic number (hologram-exec `BufferArena`). | live `run <dir> --prompt` (RSS-tracked) + `BufferArena` rotation tests | `hologram-exec buffer.rs` tests; `examples/smollm2.toml` | ✅ |
